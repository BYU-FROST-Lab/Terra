...
class MS_Map:
    ...
    def get_pcl_points_found_in_image(self, scan_idx):
        ...
        self.map_yx2idx = [dict(), dict(), dict()] # map from 2D image coord to 3D PC index
        self.map_lidar2globalidx = dict() # map from 3D lidar PC index to 3D global PC index
    ...
    def yolo_segmentation(self, scan_idx):
        ...
        self.yolo_masks = []
        for cam_idx, img in enumerate(self.camera_images):
            result = self.yolo_model(img, conf=self.yoloe_conf_thresh)
